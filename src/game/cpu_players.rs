use crate::game::{move_processor, Player};
        ...
        if player.player_move.rotations == 4 {
            ...
            match player.icon {
                '*' => {
                    player.player_move.offset_x += 1;
                    if player.player_move.offset_x > width + 2 {
                        player.player_move.offset_x = 0;
                        player.player_move.offset_y += 1;
                    }
                    if player.player_move.offset_y > length + 2 {
                        player.player_move.offset_y = 0;
                    }
                }
                '#' => {
                    player.player_move.offset_x -= 1;
                    if player.player_move.offset_x < -2 {
                        player.player_move.offset_x = width + 2;
                        player.player_move.offset_y -= 1;
                    }
                    if player.player_move.offset_y < -2 {
                        player.player_move.offset_y = length + 2;
                    }
                }
                _ => continue,
            };
